...
/* ===== BOX STATISTIK PLAYER MOBILE - AMAN ===== */
(function () {
    const BOX_ID = 'dptoto-player-stat-mobile-safe';
    const STYLE_ID = 'dptoto-player-stat-mobile-safe-css';

    let warnaIndex = 0;
    let totalDeposit = 675329522;
    let totalWithdraw = 784119020;
    let sedangBermain = 18273;
            ...
            @media screen and (max-width: 768px) {
                #${BOX_ID} {
                    ...
                    box-shadow:
                        ...
                        0 0 17px rgba(0,220,150,.70),
                        0 0 26px rgba(0,255,170,.45),
                        inset 0 0 8px rgba(255,255,255,.06)
                        !important;
                    font-family: Arial, sans-serif !important;
                    transition:
                ...
                #${BOX_ID} .mobile-stat-col {
                    min-width: 0 !important;
                }

                #${BOX_ID} .mobile-stat-right {
                    text-align: right !important;
                }

                #${BOX_ID} .mobile-stat-label {
                ...
                #${BOX_ID} .mobile-stat-winner-label {
                    font-size: 8.5px !important;
                    letter-spacing: -.15px !important;
                }

                #${BOX_ID} .mobile-stat-value {
                    margin: 0 !important;
                    color: #ffffff !important;
                    font-size: 12px !important;
                    font-weight: 800 !important;
    ...
    function buatBox() {
        ...
        box.innerHTML = `
            <div class="mobile-stat-silver"></div>

            <div class="mobile-stat-grid">
                <div class="mobile-stat-col">
                    <div
                        class="mobile-stat-label mobile-stat-winner-label"
                    >
                        Total Dimenangkan Player
                    </div>

                    <div
                        class="mobile-stat-value mobile-stat-negative"
                        id="mobile-safe-winner"
                    >
                        -Rp108.789.498
                    </div>
                </div>

                <div class="mobile-stat-col mobile-stat-right">
                    <div class="mobile-stat-label">
                        Sedang Bermain
                    </div>

                    <div
                        class="mobile-stat-value mobile-stat-active"
                    >
                        <span class="mobile-stat-dot"></span>

                        <span id="mobile-safe-active">
                            18.273
                        </span>
                    </div>
                </div>
                ...
                <div
                    class="mobile-stat-col mobile-stat-second"
                >
                    <div class="mobile-stat-label">
                        Total Deposit
                    </div>
                    ...
                    <div
                        class="mobile-stat-value"
                        id="mobile-safe-deposit"
                    >
                        Rp675.329.522
                    </div>
                </div>
                ...
                <div
                    class="mobile-stat-col mobile-stat-right mobile-stat-second"
                >
                    <div class="mobile-stat-label">
                        Total Withdraw
                    </div>
    ...
    function pasangBox() {
        ...
        /*
         * Hanya menambahkan satu elemen sebelum Hasil Terakhir.
         * Tidak mengubah isi wrapper Hasil Terakhir.
         */
        parent.insertBefore(
            box,
            hasilTerakhir
    ...
    function ubahAngka() {
        ...
        totalDeposit = angkaAcak(
            500000000,
            700000000
        );

        totalWithdraw = angkaAcak(
            750000000,
            1000000000
        );

        sedangBermain = angkaAcak(
            16000,
            22000
        );
        ...
        activeEl.textContent =
            formatID(sedangBermain);

        depositEl.textContent =
            'Rp' + formatID(totalDeposit);

        withdrawEl.textContent =
            'Rp' + formatID(totalWithdraw);
    }

    function mulai() {
        pasangBox();

        setTimeout(pasangBox, 500);
        setTimeout(pasangBox, 1500);
        setTimeout(pasangBox, 3000);

        if (window.__mobileSafeGlowInterval) {
            clearInterval(
                window.__mobileSafeGlowInterval
            );
        }

        window.__mobileSafeGlowInterval =
            setInterval(ubahGlow, 600);

        if (window.__mobileSafeNumberInterval) {
            clearInterval(
                window.__mobileSafeNumberInterval
            );
        }
