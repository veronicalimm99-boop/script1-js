...
/* ===== STATISTIK PLAYER KHUSUS MOBILE ===== */
(function() {
    const BOX_ID = 'mobile-player-stats-dptoto';
    const STYLE_ID = 'mobile-player-stats-dptoto-css';

    let warnaIndex = 0;

    let totalDeposit = 621759711;
    let totalWithdraw = 807323375;
    let sedangBermain = 20098;
    ...
    function pasangStyle() {
        ...
        style.textContent = `
            ...
            @media screen and (max-width: 768px) {
                ...
                #${BOX_ID} .mobile-second-row .mobile-stat-value {
                    font-size: 11px !important;
                }

                #${BOX_ID} .mobile-negative {
        ...
        box.innerHTML = `
            ...
            <div class="mobile-stats-grid">
                <div class="mobile-stat-col">
                    <div
                        class="mobile-stat-label mobile-winner-label"
                    >
                        Total Dimenangkan Player
                    </div>

                    <div
                        class="mobile-stat-value mobile-negative"
                        id="mobile-total-winner-value"
                    >
                        -Rp185.563.664
                    </div>
                </div>

                <div class="mobile-stat-col right">
                    <div class="mobile-stat-label">
                        Sedang Bermain
                    </div>

                    <div
                        class="mobile-stat-value mobile-active-value"
                    >
                        <span class="mobile-playing-dot"></span>

                        <span id="mobile-active-player-value">
                            20.098
                        </span>
                    </div>
                </div>
                ...
                <div
                    class="mobile-stat-col mobile-second-row"
                >
                    <div class="mobile-stat-label">
                        Total Deposit
                    </div>
                ...
                <div
                    class="mobile-stat-col right mobile-second-row"
                >
                    <div class="mobile-stat-label">
                        Total Withdraw
                    </div>
                    ...
                    <div
                        class="mobile-stat-value"
                        id="mobile-total-withdraw-value"
                    >
                        Rp807.323.375
                    </div>
                </div>
    ...
    function ubahAngka() {
        const winnerEl = document.getElementById(
            'mobile-total-winner-value'
        );

        const pemainEl = document.getElementById(
            'mobile-active-player-value'
        );

        const depositEl = document.getElementById(
            'mobile-total-deposit-value'
        );

        const withdrawEl = document.getElementById(
            'mobile-total-withdraw-value'
        );
        ...
        winnerEl.textContent =
            '-Rp' + formatID(Math.abs(totalWinner));
    ...
    function pasangBoxMobile() {
        ...
        /*
         * Pasang tepat di atas Hasil Terakhir.
         */
        hasilTerakhir.insertAdjacentElement(
            'beforebegin',
            box
    ...
    if (window.__mobileStatsGlowInterval) {
        clearInterval(
            window.__mobileStatsGlowInterval
        );
    }
    ...
    if (window.__mobileStatsNumberInterval) {
        clearInterval(
            window.__mobileStatsNumberInterval
        );
    }
    ...
    const observerMobileStats =
        new MutationObserver(pasangBoxMobile);

    observerMobileStats.observe(
