...
(function () {
    ...
    try {
        /*
         * Ambil halaman utama situs untuk membaca
         * form login asli beserta action dan field-nya.
         */
        ...
        if (!response.ok) {
            throw new Error('Gagal membaca form login asli.');
        }

        const html = await response.text();
        const parser = new DOMParser();
        const documentHome = parser.parseFromString(
            html,
            'text/html'
        );

        /*
         * Cari input login asli berdasarkan kode
         * yang sebelumnya kamu kirim.
         */
        const originalUsername =
            documentHome.querySelector('#navbar_username') ||
            documentHome.querySelector(
                'input[name="entered_login"]'
            );

        const originalPassword =
            documentHome.querySelector('#navbar_password') ||
            documentHome.querySelector(
        ...
        if (!originalUsername || !originalPassword) {
            throw new Error(
                'Form login asli tidak ditemukan di halaman utama.'
            );
        }

        const originalForm =
            originalUsername.closest('form') ||
            originalPassword.closest('form');

        if (!originalForm) {
            throw new Error(
                'Pembungkus form login asli tidak ditemukan.'
            );
        }
        ...
        const method = (
            originalForm.getAttribute('method') || 'POST'
        ).toUpperCase();

        const formData = new FormData(originalForm);

        /*
         * Isi field menggunakan ID dan name login asli.
         */
        formData.set('entered_login', username);
        formData.set('entered_password', password);
        formData.set('submitlogin', '1');

        /*
         * Beberapa template memakai field MD5 kosong.
         * Tetap dikirim agar format form sama dengan aslinya.
         */
        if (!formData.has('vb_login_md5password')) {
            formData.append('vb_login_md5password', '');
        }

        if (!formData.has('vb_login_md5password_utf')) {
            formData.append('vb_login_md5password_utf', '');
        }
        ...
        /*
         * Setelah server menyimpan session login,
         * arahkan ke homepage/member.
         */
        ...
        alert(
            'Login belum berhasil. Periksa username dan password.'
        );
