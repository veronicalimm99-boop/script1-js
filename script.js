...
/* FIX 3 TOMBOL SIDEBAR HILANG DI HALAMAN PROMOTIONS */
(function() {
    function tambahMenuSidebarDesktopFix() {
        const allItems = document.querySelectorAll('.beforesidebar__menu-item, a, button');
        if (!allItems.length) return;
        ...
        const parent = promosiItem.parentNode;
        if (!parent) return;

        ['extra-sidebar-livescore', 'extra-sidebar-rtp', 'extra-sidebar-bukti'].forEach(function(id) {
            const old = document.getElementById(id);
            if (old) old.remove();
        });
        ...
        if (typeof window.__sidebarGlowColorInterval !== 'undefined') {
            // glow lama tetap jalan
        }
    }
